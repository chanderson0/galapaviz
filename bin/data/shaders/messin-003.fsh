...
float sphere(vec3 pos)
{
    ...
    float radius = max(0.0, pow(rms, 0.5) - 0.1) * 0.5;
...
{
    float d1 = sphereRep(pos, vec3(1.0));
...
vec4 rayMarch(vec3 start, vec3 dir, vec2 screenPos) {
    ...
    if (dist < EPSILON)
    {
        ...
        float diffuse = max(0.9, dot(-dir, normal));
        ...
        vec3 myColor = mix(vec3(0,0,0), thisColor, val) * color;
...
void main() {
    ...
    vec3 cameraOrigin = vec3(x * 3.0, y * 3.0, time);
